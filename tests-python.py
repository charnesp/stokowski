...
class MuxWorkspaceManager:
    ...
    async def list_workspaces(self):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.endpoint}/api/workspace/list",
    ...
    async def send_message(self, workspace_id, message, agent_id="exec", model="ollama:kimi-k2.5:cloud"):
    ...
    async def list_projects(self):
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.endpoint}/api/projects/list",
                headers=self.headers,
                timeout=30.0
            )
            response.raise_for_status()
...
async def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  default-dir                             - Affiche le répertoire de projet par défaut")
