...
The web dashboard SHALL expose a **header theme toggle** (light / dark), **persist** the choice in **localStorage**, **honour** the OS colour scheme on **first visit** when nothing is stored, apply **short transitions** on change, and ship a **light palette** with sufficient contrast.
