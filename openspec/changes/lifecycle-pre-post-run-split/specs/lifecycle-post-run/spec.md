...
- **AND** each SHALL be rendered with the lifecycle template context including `lifecycle_phase` set to `post` for post-run (see `lifecycle-template` delta)
...
- **AND** for `agent-gate`, routing markers SHALL be resolved from that same post-run output
...
- **WHEN** a workflow state has `type: agent-gate` and `post_run: true`
